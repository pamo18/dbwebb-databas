...
function showMenu() {
    console.info(
        ...
        + `  payed <invoiceid> <date>             - Mark order as paid(Betald).\n`
        + `  logsearch <str>                      - Search the log.\n`
        + `  about                                - About page.\n`
    );
}
 ...
 * @returns {void}
 */
async function payInvoice(id, payDate) {
    await eshop.invoicePaid(id, payDate);
    console.info("\nInvoice " + id + " is now marked as paid!\n");
}
...
async function aboutEshop() {
    ...
    str = 'Jag, Paul Moreland (PAMO18), har jobbat själv på hela eshop ' +
    'projektet: eshop1, eshop2 och eshop3.';
    console.info(`\n` + str + `\n`);
}
...
async function searchTheLogs(fltr) {
    ...
    if (fltr) {
        res = await eshop.viewLogsFiltered(fltr);
        console.info(`Here are the filtered logs for ${fltr}\n\n`);
    } else {
        res = await eshop.viewLogs();
        console.info(`Here are the last 20 logs\n\n`);
    }

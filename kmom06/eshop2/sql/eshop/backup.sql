...
INSERT INTO `logg` VALUES (1,'2019-03-12 12:18:33','Product inserted: Kaffemugg med dbwebb-tryck, pris: 69.00'),(2,'2019-03-12 12:18:33','Product inserted: Temugg med dbwebb-tryck, pris: 79.00'),(3,'2019-03-12 12:18:33','Product inserted: Kaffeblandning med dbwebb-krydda, pris: 49.00'),(4,'2019-03-12 12:18:33','Product inserted: Skiva där BTHs lärarkår sjunger, pris: 199.00'),(5,'2019-03-12 12:18:33','Product inserted: DLG Penna, pris: 500.00'),(6,'2019-03-12 12:18:33','Inventory added: 1, quantity: 7 on shelf A:101'),(7,'2019-03-12 12:18:33','Inventory added: 2, quantity: 12 on shelf B:101'),(8,'2019-03-12 12:18:33','Inventory added: 3, quantity: 200 on shelf A:201'),(9,'2019-03-12 12:18:33','Inventory added: 4, quantity: 3 on shelf C:101'),(10,'2019-03-12 12:18:33','Inventory added: 5, quantity: 5000 on shelf C:201'),(11,'2019-03-12 12:19:05','Product updated, Before: id = 1, Kaffemugg med dbwebb-tryck, 69.00,  After: id = 1, Kaffemugg med dbwebb-tryck99, 69.00'),(12,'2019-03-12 12:19:14','Product inserted: either, pris: 5.00'),(13,'2019-03-12 12:19:22','Product deleted: either'),(14,'2019-03-12 12:19:38','Product updated, Before: id = 1, Kaffemugg med dbwebb-tryck99, 69.00,  After: id = 1, Kaffemugg med dbwebb-tryck, 69.00');
...
/*!50001 VIEW `v_order_details` AS select `orders`.`id` AS `id`,date_format(`orders`.`order_created`,'%Y-%m-%d -> %H:%i:%s') AS `created`,`orders`.`kund_id` AS `kund_id`,concat(`kund`.`fornamn`,' ',`kund`.`efternamn`) AS `kund_namn`,`produkt`.`namn` AS `product_name`,`order2produkt`.`kvantitet` AS `kvantitet`,date_format(`orders`.`order_updated`,'%Y-%m-%d -> %H:%i:%s') AS `updated`,(select count(`order2produkt`.`kvantitet`) from `order2produkt` where (`orders`.`id` = `order2produkt`.`order_id`)) AS `order_rows`,`orders`.`order_status` AS `order_status` from (((`orders` left join `order2produkt` on((`order2produkt`.`order_id` = `orders`.`id`))) join `kund` on((`orders`.`kund_id` = `kund`.`id`))) left join `produkt` on((`order2produkt`.`produkt_id` = `produkt`.`id`))) order by `orders`.`id` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`user`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_orders` AS select `orders`.`id` AS `id`,date_format(`orders`.`order_created`,'%Y-%m-%d -> %H:%i:%s') AS `created`,`orders`.`kund_id` AS `kund_id`,concat(`kund`.`fornamn`,' ',`kund`.`efternamn`) AS `kund_namn`,(select count(`order2produkt`.`kvantitet`) from `order2produkt` where (`orders`.`id` = `order2produkt`.`order_id`)) AS `order_rows`,`orders`.`order_status` AS `order_status`,date_format(`orders`.`order_updated`,'%Y-%m-%d -> %H:%i:%s') AS `updated`,date_format(`orders`.`order_placed`,'%Y-%m-%d -> %H:%i:%s') AS `placed`,date_format(`orders`.`order_sent`,'%Y-%m-%d -> %H:%i:%s') AS `sent`,date_format(`orders`.`order_deleted`,'%Y-%m-%d -> %H:%i:%s') AS `deleted` from ((`orders` left join `order2produkt` on((`order2produkt`.`order_id` = `orders`.`id`))) join `kund` on((`orders`.`kund_id` = `kund`.`id`))) group by `orders`.`id` order by `orders`.`id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-03-12 13:20:20
